"""
Page 1: Market View — 완전 재설계
각 차트 독립 필터 / 기간 선택 차트별 / 사이드바 고정 없음
크레딧 스프레드: 듀얼축 (두 금리선 + 그림자 스프레드 영역) + 토글
커브+MoM: 듀얼축 (bp 막대 + 금리 꺾은선) — PPT 스타일
상단 요약 테이블 (PPT 표 형식)
"""
...
GREEN_SHADES = ['#2D3F38','#4A5E35','#4E9B5A','#8DC175','#8DD5C8',
                '#DDE8C0','#8DB8A5','#9A7085','#8A9E96','#B0BDB4']
...
def _date_filter(key, df, default_days=365):
    ...
    with c1:
        preset = st.selectbox("기간", list(presets.keys()),
                              index=list(presets.keys()).index('1년'), key=f'{key}_preset')
    with c2:
        days = presets[preset]
...
def _render_summary_table(df):
    ...
    with cf2:
        ref_cat = st.selectbox("스프레드 기준 계열", all_cats,
            index=next((i for i,c in enumerate(all_cats) if "공사/공단채 AAA" in c), 0),
            key="table_ref")
...
def _render_spread_chart(df):
    ...
    with c2:
        cat_b = st.selectbox("계열 B (기준)", all_cats,
            index=next((i for i,c in enumerate(all_cats) if "공사/공단채 AAA" in c), 0), key="sp_cat_b")
    with c3:
    ...
    extra = st.multiselect("추가 계열 A 선택", [c for c in all_cats if c != cat_a],
...
def _single_curve_mom(df, cat, d1_str, d2_str):
    ...
    if len(cv1) > 0:
        ...
        fig.add_trace(go.Scatter(
            x=cv1f["tenor"], y=cv1f["yield"],
            name=f"{d1_str} (우축)",
            mode="lines+markers",
            line=dict(color="#4A5E35", width=2),
    ...
    if len(cv2) > 0:
        ...
        fig.add_trace(go.Scatter(
            x=cv2f["tenor"], y=cv2f["yield"],
            name=f"{d2_str} (우축)",
            mode="lines+markers",
            line=dict(color="#9E9E9E", width=1.8, dash="dot"),
...
def _render_timeseries(df):
    ...
    for i, cc in enumerate(ts_cats):
        ...
        if len(s) == 0: continue
        fig.add_trace(go.Scatter(
            x=s["date"], y=s["yield"], name=f"{cc} ({ts_tenor})",
            line=dict(color=COLORS_LINE[i % len(COLORS_LINE)], width=2),
